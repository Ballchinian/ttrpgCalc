...
export const manageOffGuardFrontend = (target, sourceCondition, action = "add") => {
    ...
    if (action === "add") {
        ...
        const newEffects = effects.some(e => e.name === OFF_GUARD)
            ? effects
            : [...effects, { name: OFF_GUARD, number: 1, description: `You're distracted or otherwise unable to focus your full attention on defense. You take a –2 circumstance penalty to AC.` }];
    ...
    if (action === "remove") {
        ...
        const newEffects = newSources.length === 0
            ? effects.filter(e => e.name !== OFF_GUARD)
            : effects;
        return { ...target, offGuardSources: newSources, effects: newEffects };
    }
